...
# Limpeza e padronizacao de entidades -------------------------------------

# Carregar cache
filtered_entities <- readRDS("filtered_entities.rds") %>% as_tibble()
...
adjacent_matrix <- adjacent_matrix %>% 
  mutate_all(~case_when(
    ...
    str_detect(.x, "(senador\\s)?(omar(\\sa[zs]i[zs])?|(omar\\s)?a[zs]i[zs])") ~ "@omarazizsenador",
...
# Salvar grafo
ggsave(filename = 'grafo.png', width = 12, height = 10, device='png', dpi=700)
